...
/* Change these values to customize the maze visualization. */
...
#define pathColor		RGB(0x10,0x10,0x10)
...
/* maze[i][j] =  (0~3) visited by k              (If maze[i][j] is a room visited from maze[i-di[k]][j-dj[k]].)
 *               (4) startingposition            (If maze[i][j] is a room and is the starting position.)
 *               (5) undiscovered	             (If maze[i][j] is a room and not discovered.)
 *				 (6) wall		                 (If maze[i][j] is a wall.)
 *				 (7~10) discovered by (wall+k+1) (If maze[i][j] is a room discovered from maze[i-di[k]][j-dj[k]]) */
#define startingposition 4
#define undiscovered	 5
#define wall		     6
std::vector<std::vector<char> > maze;
int width, height;

/* mazeMarked: true if the trace is marked on the maze. */
...
void freeMemory(){
	...
	/* The only dynamically allocated memory is the maze.
	 * Use vector.swap() function to reallocate memory. */
...
bool readFile(const char* filename){
     ...
     * and rooms (' ' in the file) are marked UNDISCOVERED in MAZE. */
	char cBuff;
	FILE * fp;

    /* This must be true because
	 * freeMemory() empties the vector after every file close. */
	ASSERT(maze.empty());
...
void DFS() {
    ...
    maze[iStart][jStart] = startingposition;
	/* Start DFS. */
	while (!s.empty()) {
		/* Pop from stack. */
		int i = s.top().first;
		...
		for(int k = 0; k < 4; k++) {
			...
			if(0 <= iNext && iNext < height
				&& 0 <= jNext && jNext < width) {
					/* If maze[iNext][jNext] is an undiscovered room */
					if(maze[iNext][jNext] == undiscovered) {
						/* Push the position to stack and mark the cell as discovered. */
...
void BFS() {
	...
	/* Enqueue the starting position to queue. */
		...
		for(int k = 0; k < 4; k++) {
			...
			if(0 <= iNext && iNext < height
				&& 0 <= jNext && jNext < width) {
					/* If maze[iNext][jNext] is an undiscovered room */
					if(maze[iNext][jNext] == undiscovered) {
						/* Enqueue the position to queue and mark the room as discovered. */
						q.push(std::make_pair(iNext, jNext));
						maze[iNext][jNext] = k+wall+1;
					}
			}
		}
	}
    showMessage("BFS success");
	//end of user code
}
...
static void drawBuffered(){
	...
	if(0 <= maze[iEnd][jEnd] && maze[iEnd][jEnd] < 4) {
		int i = iEnd;
		int j = jEnd;
...
static void drawLineSegment(int i0, int j0, int i1, int j1, const COLORREF color) {
	/* This function is used only to connect two adjacent rooms or same rooms.
