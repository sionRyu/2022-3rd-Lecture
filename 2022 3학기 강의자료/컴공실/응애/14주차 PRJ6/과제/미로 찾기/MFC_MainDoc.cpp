...
BEGIN_MESSAGE_MAP(CMFC_MainDoc, CDocument)
	...
	ON_COMMAND(ID_TOOLS_RESET, &CMFC_MainDoc::OnToolsReset)
	...
	ON_UPDATE_COMMAND_UI(ID_TOOLS_RESET, &CMFC_MainDoc::OnUpdateToolsReset)
END_MESSAGE_MAP()
...
void CMFC_MainDoc::OnToolsReset()
{
	// execute menu handler function
	eraseMark();
...
void CMFC_MainDoc::OnUpdateToolsReset(CCmdUI *pCmdUI)
{
	// if user data is loaded, enable "Reset" menu
	pCmdUI->Enable(m_bDataLoaded);
}
